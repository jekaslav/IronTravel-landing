...
if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
        ...
        let telegramMessage = `Новая заявка на тур%0A%0AИмя: ${name}%0AМаршрут: ${route}`;
        if (message && message.trim()) {
            telegramMessage += `%0A%0AСообщение:%0A${message}`;
        }
        
        // Отправляем в Telegram
        window.open(`https://t.me/irontravel_ir?text=${telegramMessage}`, '_blank');
